...
def create_features(
    ...
    Args:
        ...
        target (bool): boolean which indicates if target values for training are provided in the dataframe. if True, df should contain additionally one column for each entity in 'entity_list', which are binary columns indicating a sentence is relevant (1) for set entity, or not (0).
    ...
    if target is False:
        all_regex_list, all_regex_dict = preprocess_regex(entity_list, regex_dict)
        # create features
        df_features, feature_list = add_features(df, all_regex_list)
        # new dataframe both containing targets and features:
        df_features = df_features.reset_index(drop=True)

    # return
    return df_features, all_regex_dict, feature_list
